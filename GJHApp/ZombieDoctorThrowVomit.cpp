...
#include "ZombieDoctorThrowVomit.h"
#include "InstanceObject.h"

void ZombieDoctorThrowVomit::Start()
{
	m_HP = 8;
	m_MoveSpeed = 400.f;
...
void ZombieDoctorThrowVomit::Update()
{
	if (ColMap(50) == true)
	{
		CreateEndEffect();
...
void ZombieDoctorThrowVomit::ColEnter(GJHCollision* _This, GJHCollision* _Other)
{
...
void ZombieDoctorThrowVomit::CreateEndEffect()
{
	InstanceObjectData Data;
	Data.Size = GJHVector(256, 256);
	Data.Pos = GetPos();
...
void ZombieDoctorThrowVomit::SetAnimationData()
{
	float Delay = 0.06f;
	int XGap = 0;
	int YGap = 0;
	PushAnimationData("ThrowVomit", 12, -10 + XGap, 0 + YGap, Delay);
	PushAnimationData("ThrowVomit", 13, -10 + XGap, 5 + YGap, Delay);
	PushAnimationData("ThrowVomit", 14, -2 + XGap, 5 + YGap, Delay);
	PushAnimationData("ThrowVomit", 15, -5 + XGap, 5 + YGap, Delay);
	PushAnimationData("ThrowVomit", 16, -6 + XGap, 5 + YGap, Delay);
	PushAnimationData("ThrowVomit", 17, 5 + XGap, 5 + YGap, Delay);
	PushAnimationData("ThrowVomit", 18, -11 + XGap, 4 + YGap, Delay);
	PushAnimationData("ThrowVomit", 19, -10 + XGap, 2 + YGap, Delay);
	PushAnimationData("ThrowVomit", 20, -5 + XGap, 5 + YGap, Delay);
	PushAnimationData("ThrowVomit", 21, 0 + XGap, -10 + YGap, Delay);
	PushAnimationData("ThrowVomit", 22, 0 + XGap, -5 + YGap, Delay);
	PushAnimationData("ThrowVomit", 23, 5 + XGap, -10 + YGap, Delay);
	PushAnimationData("ThrowVomit", 24, 5 + XGap, 0 + YGap, Delay);
	PushAnimationData("ThrowVomit", 25, 5 + XGap, -10 + YGap, Delay);
}